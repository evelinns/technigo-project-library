window.onload = (event) => {
  ...
  // Changing the formatting of the cuisin types
  ...
  // Formatting the ingredients from the array
  ...
  // Creating buttons for each cuisine type
  const generateCuisineButtons = () => {
    const buttonContainer = document.getElementById("kitchen-button-container");
    ...
    recipes.forEach((recipe) => {
      if (Array.isArray(recipe.cuisineType)) {
        ...
        recipe.cuisineType.forEach((type) =>
          cuisineTypes.add(type.toLowerCase())
        );
      } else {
        cuisineTypes.add(recipe.cuisineType.toLowerCase());
      }
    });

    // Generate buttons for each cuisine type
    cuisineTypes.forEach((type) => {
      const button = document.createElement("button");
      button.classList.add("kitchen-filters");
      button.setAttribute("data-cuisine", type);
      button.textContent = capitalizeAndFormat(type);
  ...
  // Create the HTML card for each recipe
  const createRecipeCard = (recipesToRender, filterCuisine, sortTime) => {
    const recipeCard = recipesToRender
      .filter((el) => {
        if (!filterCuisine || filterCuisine === "all") {
          return true;
        } else {
          return (
            el.cuisineType.indexOf(filterCuisine) >= 0 ||
            el.cuisineType === filterCuisine
          );
        }
      })
      .sort((a, b) => {
  ...
  createRecipeCard(recipes, null);

  let currentCuisine;
  let sortedTime;

  // Styling of button clicked to showcase it being active
  const buttonToggleCuisine = (buttonClicked) => {
    buttonClicked.forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        createRecipeCard(recipes, currentCuisine, sortedTime);
      });
    });
  };

  // Styling of button clicked to showcase it being active
  const buttonToggleTime = (buttonClicked) => {
    buttonClicked.forEach((btn) => {
      btn.addEventListener("click", (e) => {
        buttonClicked.forEach((f) => f.classList.remove("active"));
        e.target.classList.toggle("active");
        sortedTime = btn.getAttribute("data-sort");
        createRecipeCard(recipes, currentCuisine, sortedTime);
      });
    });
  };

  buttonToggleCuisine(kitchenButtonToggle);
  buttonToggleTime(timeButtonToggle);
};
